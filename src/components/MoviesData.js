...
class MoviesData extends React.Component {
  ...
  parseFilmsJson = () => {
    ...
    allFilms.forEach((film) => {
      const planets = allPlanets.filter(function (planet) {
        return planet.filmConnection.films.find((el) => {
  ...
  getImageName = (filmId) =>
    this.shouldDisplay(filmId) ? "closeArrow" : "openArrow";

  shouldDisplay = (movieId) =>
    this.state.openSections[movieId] ? true : false;
        ...
        {this.state.films.map((film) => {
          return (
            <div key={film.id}>
              ...
              {this.shouldDisplay(film.id) && (
                <PlanetsData planets={film.planets} />
              )}
            </div>
          );
        })}
